...
contract Vault is IVault, ERC4626 {
    ...
    function recoverCollateral(uint256 fromIndex, uint256 _amt) external {
        _onlyAdminOrSwapper();
        if (AUTH.paused()) revert VaultPaused();

        (uint256 totalValue) = AVL.recoverCollateral(
            INFO,
            POOL,
            fromIndex,
            _amt,
        ...
        removedCollateralValue += totalValue;

        emit RecoverCollateral(msg.sender, fromIndex, totalValue);
    }
